	...
	//7월23일
		...
		public @ResponseBody String refuseFriendByAjax(String friendId, String userId) {
		 ...
		 @RequestMapping(value="/moveUserInfoModifyForm")
		   ...
		   //프리미엄 결제 페이지 이동
		   @RequestMapping(value="/movePremiumService")
			public ModelAndView movePremiumService(){
				String loginId = (String)session.getAttribute("loginId");
				mav = msvc.premiumMemberInfo(loginId);
				return mav;
			}

		   //프리미엄 결제 후
		   @RequestMapping(value = "/premiumMember")
		   public ModelAndView premiumMember(String loginId){
		      mav = msvc.premiumMember(loginId);
		      return mav;
		   }
}
