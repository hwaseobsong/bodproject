...
public interface BoardDao {
	...
	@Select("SELECT BMID, BDATE, BCONTENT, BCODE, BVIDEO, MPROFILE, MNICK AS BMNICK, BGCODE FROM BOARD, MEMBER "
			+ "WHERE BMID = MID AND BCODE IN( "
			+ "SELECT BCODE FROM BOARD "
			+ "WHERE BCODE IN(SELECT BCODE FROM BOARD WHERE BMID = #{loginId}) "
			+ "OR BCODE IN (SELECT BCODE FROM BOARD WHERE BMID IN(SELECT FRMID FROM FRIEND WHERE MYMID = #{loginId})) "
			+ "OR BCODE IN (SELECT BCODE FROM BOARD WHERE BGCODE IN (SELECT GUGCODE FROM GUSER WHERE GUMID = #{loginId})) "
			+ "GROUP BY BCODE) ORDER BY BDATE DESC")
	...
	@Select("SELECT BMID, BDATE, BCONTENT, BCODE, BVIDEO, MPROFILE, MNICK AS BMNICK, BGCODE FROM BOARD, MEMBER WHERE BMID = MID AND BGCODE = #{gcode} ORDER BY BDATE DESC")
	...
	@Select("SELECT BMID, BDATE, BCONTENT, BCODE, BVIDEO, MPROFILE, MNICK AS BMNICK, BGCODE FROM BOARD, MEMBER WHERE BMID = MID AND MID IN (SELECT MID FROM MEMBER WHERE MID = #{loginId}) ORDER BY BDATE DESC")
	...
	@Select("SELECT BMID, BDATE, BCONTENT, BCODE, BVIDEO, MPROFILE, MNICK AS BMNICK, BGCODE FROM BOARD, MEMBER WHERE BMID = MID AND MID IN (SELECT MID FROM MEMBER WHERE MID = #{userId}) ORDER BY BDATE DESC")
	...
	@Select("SELECT MNICK, MPROFILE, MID FROM MEMBER WHERE MID IN(SELECT LIMID FROM BOARDLIKE WHERE LICODE = #{code})")
	ArrayList<BoardLikeDto> getLikeList(String code);
}
