...
public interface GroupDao {
	...
	//그룹 탈퇴시
	   ...
	   @Select("SELECT GNAME FROM GROUPPAGE WHERE GCODE IN (SELECT GUGCODE FROM GUSER WHERE GUMID = #{loginId}) "
		         +" ORDER BY DBMS_RANDOM.RANDOM")
	   ArrayList<String> getUserGroupName(String loginId);

	   @Select("SELECT GR.GCODE, GR.GNAME, GR.GINFO, GR.GPROFILE, GR.GMANAGER, GU.GCOUNT AS GUSERNUM "
						+ " FROM GROUPPAGE GR, (SELECT COUNT(GUMID) AS GCOUNT, GUGCODE FROM GUSER "
						+ " GROUP BY GUGCODE) GU WHERE ( GR.GCODE = GU.GUGCODE) "
						+ " AND NOT GR.GCODE IN (SELECT GUGCODE FROM GUSER WHERE GUMID = #{loginid} ) AND (GR.GNAME LIKE #{gname} OR GR.GINFO LIKE #{gname} ) "
						+ " ORDER BY DBMS_RANDOM.RANDOM")
		ArrayList<GroupPageDto> recomGroup(@Param("gname")String randomName,@Param("loginid") String loginId);
